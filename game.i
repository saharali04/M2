...
void initSnake() {
    ...
    snakeHead.width = 8;
    snakeHead.height = 8;
    snakeHead.col = 240/2 - (snakeHead.width/2) - 40;
...
void updateSnake() {
    ...
    shadowOAM[0].attr1 = snakeHead.col | (0<<14);
    shadowOAM[0].attr2 = ((snakeHead.curFrame)*32+(snakeHead.aniState));


    for (int i = 0; i < 20; i++) {

        if (snakeBodyArray[i].active) {
            shadowOAM[i+33].attr0 = snakeBodyArray[i].row | (0<<13) | (0<<14);
            shadowOAM[i+33].attr1 = snakeBodyArray[i].col | (0<<14);
            shadowOAM[i+33].attr2 = ((0)*32+(snakeBodyArray[i].aniState));
...
void updateGame() {
    ...
    if (collision(snakeHead.col, snakeHead.row, snakeHead.width, snakeHead.height, apple.col, apple.row, apple.width, apple.height)) {
        ...
        shadowOAM[2].attr2 = ((0)*32+(BLACKBACKGROUND));
        DMANow(3, shadowOAM, ((OBJ_ATTR*)(0x7000000)), 128 * 4);
        apple.active = 0;
